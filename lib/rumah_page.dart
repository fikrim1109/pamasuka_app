...
class _RumahPageState extends State<RumahPage> {
  ...
  final TextEditingController _tokoController = TextEditingController(); // Untuk Tanggal Survei
  ...
  // Anda bisa menambahkan flag isSuperUser di sini jika diperlukan untuk logika UI
  ...
  void initState() {
    ...
    // _namaController.text = widget.username; // DIHAPUS: Biarkan user input, atau ada logika khusus
    // Contoh: Cek jika user adalah superuser, maka set default _namaController.text = widget.username
    // _isSuperUser = widget.userId == 785; // Contoh ID superuser
    // if (_isSuperUser) {
  ...
  void _showStyledSnackBar(String message, {bool isError = false}) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(message), // Style diambil dari SnackBarTheme di AppTheme
        backgroundColor: isError ? AppSemanticColors.danger(context) : AppSemanticColors.success(context),
        // behavior, shape, margin sudah diatur di AppTheme
      ),
    );
  }
  ...
  void _resetForm() {
    ...
    setState(() {
      _namaController.clear(); // Clear nama surveyor
      ...
      // Biarkan _fetchOutlets yang menangani field outlet
      _tokoController.text = DateFormat('yyyy-MM-dd').format(DateTime.now());
      // Jika ada logika superuser untuk nama default, bisa ditambahkan di sini juga
      // if (_isSuperUser) {
      //   _namaController.text = widget.username;
      // }
    });
    _fetchOutlets(); // Refresh outlet
  }
  ...
  Future<void> _submitForm({bool confirmDuplicate = false}) async {
    ...
    // Validasi Nama Surveyor tidak boleh kosong (sudah ada di _buildTextField validator, tapi double check)
    ...
    request.fields['username'] = widget.username; // Username login
    
    // Kirim nama_surveyor dari _namaController
 ...
 @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Formulir Survei'), // Judul AppBar
        centerTitle: true,
      ),
      body: SafeArea(
        child: Center(
                                ...
                                _buildTextField(
                                  ...
                                  validator: (value) {
                                    ...
                                    if (value.trim().length < 3) {
                                      return 'Nama surveyor minimal 3 karakter';
                                    }
                                    return null;
                                  }
                                ),
                                const SizedBox(height: 16), // Beri jarak setelah nama surveyor
                                                ...
                                                Row(
                                                  ...
                                                  children: [
                                                    ...
                                                    TextButton.icon(
                                                      ...
                                                      label: Text(isHidden ? 'Tampilkan' : 'Sembunyikan', style: theme.textTheme.labelMedium),
                                                ...
                                                if (!isHidden) ...[
                                                  ...
                                                  Padding(
                                                    ...
                                                    child: DropdownButtonFormField<String>(
                                                      ...
                                                      decoration: InputDecoration(
                                                        labelText: 'Jenis Paket * (Wajib jika ada data harga)',
                                                      ),
                        ...
                        child: Container(
                          decoration: BoxDecoration(
                            ...
                            borderRadius: (theme.cardTheme.shape is RoundedRectangleBorder && 
                                           (theme.cardTheme.shape as RoundedRectangleBorder).borderRadius != null)
